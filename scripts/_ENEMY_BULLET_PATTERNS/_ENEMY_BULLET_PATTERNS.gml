function _ENEMY_BULLET_CIRCLE_PATTERN(_bullet_amount, _speed, _radius) {
...
function _ENEMY_BULLET_FLOWER_PATTERN(_bullet_amount, _speed, _radius, _point_amount, _flower_depth) {
...
function _ENEMY_BULLET_STAR_PATTERN(_bullet_amount, _initial_speed, _minimal_speed, _radius, _point_amount, _depth) {
...
function _ENEMY_BULLET_BARRIER_PATTERN(_bullet_amount, _speed, _spread, _homing) {
	...
	for (var i = 0; i < _bullet_amount; i++) {
		...
		with instance_create_layer(x, y, "Enemy_Projectiles", obj_enemy_bullet) {
			image_angle = _dir;
			direction = _dir;
			speed = _speed;
		}
